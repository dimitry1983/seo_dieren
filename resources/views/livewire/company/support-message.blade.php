    ...
    <div class="main-container flex relative">
        ...
        <div class="main-content flex-1 px-5 py-10 lg:p-16 max-w-[100%]">
            ...
            <div class="border border-solid border-[#CFD7E4] rounded-[15px] px-5 py-6 lg:px-16 lg:py-9">
                <h2 class="text-xl font-bold mb-4">{{__('Support')}}</h2>
                <p>{{__('Hulp nodig? Neem via het onderstaande formulier contact met ons.')}}</p>
                <form wire:submit.prevent="save">
                    <textarea wire:model.defer="message" class="w-full border border-gray-300 p-2 rounded-lg" placeholder="{{__('Typ jouw bericht...')}}" rows="5"></textarea>
                    @error('message') <span class="text-red-500">{{ $message }}</span> @enderror
                    <div class="mt-4">
                        <x-filament::button type="submit">{{__('Bericht verzenden')}}</x-filament::button>
                    </div>
                </form>
            </div>
        </div>
