                ...
                <div class="w-full border border-gray-300 bg-white py-4 rounded-lg mb-8">
                    ...
                    <div class="fi-section-content-ctn border-t py-4 border-gray-200 dark:border-white/10" id="addPrice">
                        <div x-data="{ confirmDelete: false, priceToDelete: null }">
                            ...
                            <ul class="space-y-2 px-4 min-w-[420px]">
                                    @if (!empty($this->prices[0]))
                                        @foreach($this->prices as $price)
                                            <li class="flex items-center justify-between odd:bg-gray-100 even:bg-white dark:odd:bg-[#1d1d1d] dark:even:bg-[#2d2d2d] p-3 rounded">
                                                <!-- Left: Name & Animal -->
                                                <div class="flex items-center space-x-4">
                                                    <span class="font-medium w-48 px-4">{{ $price->name }}</span>
                                                    <span class="font-medium w-32 px-4">{{ $price->animal_name }}</span>
                                                </div>
                                                ...
                                                <div class="flex items-center space-x-4">
                                                    ...
                                                    <!-- Edit Button -->
                                                    <button class="text-primary hover:text-blue-600 ml-2" wire:click="loadPrice({{ $price->id }})">
                                                        <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zM19.5 7.125L16.862 4.487" />
                                                        </svg>
                                                    </button>

                                                    <!-- Delete Button -->
                                                    <button @click="priceToDelete = {{ $price->id }}; confirmDelete = true" class="text-red-500 hover:text-red-600 ml-2">
