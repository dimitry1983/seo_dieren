<div>
    ...
    <div class="main-container flex  relative">
        ...
        <div class="main-content flex-1  px-5 py-10 lg:p-16 max-w-[100%]">
            <div wire:loading wire:target="save, deleteImage" class="fixed inset-0 flex items-center justify-center bg-white bg-opacity-35 z-50">
                    ...
                    <div class="fi-section-content-ctn border-t py-4 border-gray-200 dark:border-white/10" id="addNews">
                        <div x-data="{ confirmDelete: false, blogToDelete: null }">
                            <ul class="space-y-2 px-4">
                                    ...
                                    @foreach($blogs as $blog)
                                        <li class="flex items-center justify-between">
                                            ...
                                            <div class="w-16 h-16 mr-4">
                                                <img src="{{ Storage::url($blog->thumb) }}" alt="{{ $blog->name }}" class="w-full h-full object-cover rounded-lg">
                                            </div>
                                            ...
                                            <div class="flex items-center space-x-4">
                                                <!-- Edit Button -->
                                                <button class="text-primary hover:text-primary" wire:click="loadBlog({{ $blog->id }})">
                                                ...
                                                <!-- Delete Button -->
                                                <button @click="blogToDelete = {{ $blog->id }}; confirmDelete = true" class="text-red-500 hover:text-red-600">
                                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
                                ...
                                @else    
                                    <x-alert-box>
                                        {{__('Er zijn nog geen blogs toegevoegd.')}}
                                    </x-alert-box>
                                @endif
                            </ul>
                            ...
                            <div x-show="confirmDelete" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50" x-cloak>
                                <div class="bg-white p-4 rounded-lg shadow-lg">
                                    ...
                                    <div class="flex justify-end space-x-2">
                                        ...
                                        <button @click="$wire.deleteBlog(blogToDelete); confirmDelete = false" class="px-4 py-2 bg-red-500 text-white rounded-md">{{ devTranslate('media.Ja, verwijderen', 'Ja, verwijderen') }}</button>
                                    </div>
                                </div>
                            </div>
                        </div>
        ...
        document.addEventListener("saved", function() {
                ...
                setTimeout(() => {
                    ...
                    setTimeout(() => {
                        ...
                        setTimeout(() => {
                            successMessage.remove(); // Remove after fade out
                            window.location.reload(); // Refresh the page after the message is gone
